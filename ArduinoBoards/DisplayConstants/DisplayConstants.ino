...
#define _FILENAME_ String(__FILE__).substring(String(__FILE__).lastIndexOf('/')+1)
void setup()
{
  Serial.begin(9600);
  ...
  LINE("__FILE__", __FILE__);
   ...
   String x = String(__FILE__);
   String y = x.substring(x.lastIndexOf('/')+1);
//   String y = String(__FILE__).substring(String(__FILE__).lastIndexOf('/')+1);
   
  // filename
  LINE("filename", y);

  // filename
  LINE("filename", _FILENAME_);
